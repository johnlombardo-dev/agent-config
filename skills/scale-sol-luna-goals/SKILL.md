...
- The orchestrator owns the user goal, roadmap, Consequential Decisions, contract acceptance, implementation-worker dispatch, integration, and completion. For an indivisible `ESCALATE` outcome only, it may approve a bounded coupled-decision mandate instead of separating a named design choice from implementation when that separation would break the invariant. Approving the mandate is itself an orchestrator-owned Consequential Decision.
...
- A high-capability implementer owns an indivisible `ESCALATE` outcome, including state-chart design or implementation, at the highest-capability verified route. Its accepted escalation envelope must freeze the goal, invariants, public boundaries, scope, mutation authority, acceptance checks, and stop conditions. A bounded coupled-decision mandate may additionally grant it authority over named design questions that must remain coupled to implementation. It must stop and return any decision outside that mandate. Select reasoning effort independently as `high`, `xhigh`, or `max` under the shaping policy. Never use `ultra`, silently change the selected route, or split such work merely to make it Luna-routable.
...
3. **Shape.** Before each implementation dispatch, read [shape-luna-contract](../shape-luna-contract/SKILL.md) completely and apply it to exactly one dependency-ready outcome. Resolve Consequential Decisions in the main thread. For an indivisible escalation whose named design choices must stay coupled to implementation, approve its bounded coupled-decision mandate in the main thread instead. Admit its `READY` Luna contract or preserve its accepted `ESCALATE` envelope for the required high-capability route; feed `PREREQUISITE` or `NO-OP` back into the goal loop. If the skill is unavailable or unreadable, record a capability blocker instead of reconstructing its rules.
4. **Dispatch.** Recheck the task fingerprint; treat changed facts, decisions, ownership, constraints, dependencies, or checks as material drift requiring reshaping. Reserve mutation ownership, populate and verify the caller-owned runtime fields, and send only route-appropriate worker content. For an escalation, include the accepted envelope and any bounded coupled-decision mandate. Never send an `ESCALATE` outcome to Luna.
