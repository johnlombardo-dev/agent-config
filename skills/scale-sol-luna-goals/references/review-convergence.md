...
Read this reference for every skill invocation that produces implementation changes. It owns review-stage mechanics and the returned manifest only. Role authority, delivery modes, dispatch validity, Luna schemas, runtime mappings, persistence, and metrics remain owned by their parent sources.
...
- Review-event persistence in the canonical invocation log is authorized only while review convergence is active.

## Review events

Append review findings, proposed contracts, decisions, and results to the invocation's canonical log:

```text
~/.codex/subagent-metrics/scale-sol-luna-goals/<repository-id>/<skill-use-id>.jsonl
```

Treat these events as a redacted, reconstructible, non-authoritative coordination cache. Never store secrets, credentials, raw reasoning, or unbounded logs.

Use [`append_metric.py`](../scripts/append_metric.py) for every append. Include `stage` as `local` or `hosted-pr`. The helper adds the canonical envelope, locks the file, and enforces role-specific event types. Do not write the log directly.

```text
review_cycle_observed: cycle_id, stage, source, base, head, finding IDs and severity counts
review_finding: finding_id, cycle_id, source ID/URL, severity, title, path/range, affected scenario, evidence, candidate disposition, related finding IDs
review_contract_proposed: contract_id, cycle_id, finding_ids, profile, Failure Domain, mutation scope, depends_on, conflicts_with, checks, full proposed Luna packet
review_decision: finding|contract, target_id, accepted|rejected|superseded, reason, orchestrator fingerprint
review_contract_result: contract_id, landed|no-change|failed|aborted, evidence, resulting head, rework attribution
review_cycle_outcome: cycle_id, clean|findings|stale|blocked, unresolved IDs, checks, resulting head, and timing fields added by the helper
```

The invocation log is an event log, not a LIFO scheduler. Dispatch order comes only from orchestrator-accepted dependencies, conflicts, and mutation reservations in the current manifest.

Give the contract writer using the review-phase `xhigh` profile temporary ownership to invoke the helper with `--role writer`, which permits only `review_cycle_observed`, `review_finding`, and `review_contract_proposed` entries. After it returns, the orchestrator uses the default role for `review_decision`, `review_contract_result`, and `review_cycle_outcome` entries. Luna workers and reviewers never write the log. Missing or corrupt review events are a cache miss; reconstruct from Git, hosted review evidence, and accepted contracts.

The writer returns a compact manifest containing the cycle and exact head, every finding ID and candidate disposition, finding counts, contract IDs, Luna profiles, mutation scopes, dependencies, conflicts, ready and blocked sets, residual risks, and the invocation-log pointer. The manifest is an index, not a second copy of findings or packets.

## One review cycle

Instrument every cycle through [outcome-metrics.md](outcome-metrics.md). Append the cycle start before preparation. Append a start and finish event around each applicable `prepare`, `wait_for_hosted_review`, `review_and_shape`, `fix_and_integrate`, and `verify` step. Capture timestamps before doing the work so an interrupted cycle still has a durable start event.
...
7. Append the cycle outcome, then start a new cycle at the resulting exact head. Never infer cleanliness from the prior review.
