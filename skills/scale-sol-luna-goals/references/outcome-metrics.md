...
Read this reference and record events every time the skill is invoked. One append-only JSONL file is the canonical durable record for invocation metrics and assignment outcomes:
...
This reference owns the required observation records, routine summary, and controlled-comparison schema only. It does not change dispatch validity, precedence, verification requirements, or completion criteria in the parent `SKILL.md`.
...
- [Legacy review records](#legacy-review-records)
...
The helper adds `schema_version`, `created_at`, `repository_id`, and `skill_use_id` immediately before appending. It adds start timestamps to supported start events and calculates finish timestamps and elapsed milliseconds from their persisted start events. Current SSLG events use the default `orchestrator` role. The helper rejects event types outside the selected role. Do not write the log directly or supply generated fields yourself. If the helper cannot obtain a reliable clock or append the record, report the persistence gap instead of inventing values.
...
Every persistence helper validates this format. Resolve it once and reuse the returned string unchanged for metrics, goal state, artifacts, and continuations.
...
At each framing checkpoint, append `use_checkpoint`. Before ending the invocation, append `use_outcome`. Include:
...
The persistence scripts still accept historical `review_*` records so existing logs remain readable. Current SSLG does not emit them. Do not create review cycles, findings, contracts, manifests, or review timing records. Leave script and schema simplification to the separate metrics follow-up.

Keep the skill-use outcome active while the selected delivery mode has unfinished verification, CI, PR, or merge work. For default delivery, record `goal-verified` only after the exact verified head passes required CI and is merged. For no-PR mode, require final local verification. For no-merge mode, require final verification and green required CI, but not merge. Record a capability or authority blocker with the corresponding non-complete outcome.
...
Run [`summarize_metrics.py`](../scripts/summarize_metrics.py) at each framing checkpoint and before the final response. Do not wait for a separate user request. Report available invocation and assignment totals concisely, including unavailable telemetry.
...
Compare directly measured elapsed time by assignment phase and outcome. Report useful prerequisite or `NO-GO` work separately from failed or abandoned work.
...
- repeated checks without state change; and
- elapsed time and token classes only when directly measured.

Do not sum overlapping worker durations and call the result wall-clock time. Use directly measured invocation timing for wall-clock analysis and worker telemetry for compute or token analysis.
