...
- Sol owns research, Consequential Decisions, sequencing, contract shaping, implementation-worker dispatch, integration, and completion. For an indivisible `ESCALATE` outcome only, it may approve a bounded coupled-decision mandate instead of separating a named design choice from implementation when that separation would break the invariant. Approving the mandate is itself a Sol-owned Consequential Decision.
